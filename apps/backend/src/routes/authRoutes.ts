...
import { authRateLimit, economyMutationRateLimit, passwordRecoveryRateLimit, registrationRateLimit } from "../middleware/security.js";
...
authRoutes.post("/register", registrationRateLimit, validateBody(registerSchema), asyncHandler(register));
authRoutes.post("/login", authRateLimit, validateBody(loginSchema), asyncHandler(login));
authRoutes.post("/forgot-password", passwordRecoveryRateLimit, validateBody(forgotPasswordSchema), asyncHandler(forgotPassword));
authRoutes.post("/reset-password", passwordRecoveryRateLimit, validateBody(resetPasswordSchema), asyncHandler(resetPasswordController));
...
authRoutes.post("/guest-transfer", requireAuth, economyMutationRateLimit, validateBody(guestTransferSchema), asyncHandler(guestTransfer));
