...
export async function getAdminAnalytics(): Promise<AdminAnalyticsDto> {
  ...
  const [
    ...
    prisma.user.count({ where: { trustStatus: "SUSPICIOUS", status: "ACTIVE" } }),
    prisma.user.count({ where: { emailVerifiedAt: null, status: "ACTIVE" } }),
    prisma.supportTicket.count({ where: { status: "OPEN" } }),
    prisma.score.count({ where: { status: { in: ["suspicious", "pending_review"] } } }),
    prisma.purchaseTransaction.count({ where: { createdAt: { gte: thirtyDaysAgo } } }),
